...
if isequal(riverChoice,'fractal')
    ...
    riverHolder=readshape2('SF1',3,1);
end

river=zeros(size(riverHolder,1)+bufferZone,size(riverHolder,2)+bufferZone);
landGrid=ones(size(riverHolder,1)+bufferZone,size(riverHolder,2)+bufferZone);
for level=1:2
...
landGrid=landGrid+river.*landToWaterRatio;
river;
%generate non-flow prob values of movement
[riverWeight,rows,columns,landGridWeight]=weight(river,landGrid);
...
while(run)
    ...
    elseif isequal(state,'larvae')
        ...
        [movers,timeSlice,histo,state,cycleCounter,run,time]=adultMotion(movers,adultTime,landGridWeight,histo,timeSlice,cycleCounter,run,time,cyclesDesired,river);
    end
end
% tic
for i=1:time
