...
[baseNodesWithoutDiagonalEntries]=baseNodeCreation(numberOfInstancesRiverSplits);
dims=peak2peak(baseNodesWithoutDiagonalEntries);%river size based on both coordinates?
river = zeros(dims(2),dims(1));%matrix used to shift node values >= 0
set=size(river,1);%first river size, second dimension?

%shifts the values to all positive from being centered around the y-axis
for nodeCounterFromBaseNodes=1:size(baseNodesWithoutDiagonalEntries,1)
    river(dims(2)-baseNodesWithoutDiagonalEntries(nodeCounterFromBaseNodes,2)+1,baseNodesWithoutDiagonalEntries(nodeCounterFromBaseNodes,1)+dims(1)/2+1)=1;
end
...
centerXValues(1)=dims(1)/2+regionRadiusFromCentralRiver+1;%centers on the middle of the main branch
for yCoord=size(riverRegion,1)-regionRadiusFromCentralRiver:-1:11+regionRadiusFromCentralRiver
    if 0==mod(yCoord-(1+regionRadiusFromCentralRiver),20)
        for centerXValueCounter=1:size(centerXValues,2)
            j0=centerXValues(centerXValueCounter);
            box=regionRadiusFromCentralRiver+10*2^(splitPower-1)-1;
